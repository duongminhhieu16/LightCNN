...
class network_layers_test(nn.Module):
    def __init__(self, num_classes=79077):
        super(network_layers_test, self).__init__()
        self.features = nn.Sequential(
            mfm(1, 48, 5, 1, 2), 
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True), 
            group(48, 96, 3, 1, 1), 
        ...
        self.fc1 = mfm(16*16*128, 64, type=0)
...
##            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True),
##            )
##        self.fc1 = mfm(4*4*128, 64, type=0)
##        self.fc2 = nn.Linear(64, num_classes)
##
##    def forward(self, x):
##        x = self.features(x)
...
def LightCNN_test(**kwargs):
    model = network_layers_test(**kwargs)
    return model
